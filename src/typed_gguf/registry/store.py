...
`TYPED_GGUF_HOME` overrides the whole base directory.
...
from typed_gguf.errors import ModelNotFoundError, RegistryCorruptError

SCHEMA = "typed_gguf.registry/v1"
...
def data_home() -> pathlib.Path:
    ...
    raw = os.environ.get("TYPED_GGUF_HOME")
    if raw:
        return pathlib.Path(os.path.expanduser(raw))
    xdg = os.environ.get("XDG_DATA_HOME")
    if xdg:
        return pathlib.Path(os.path.expanduser(xdg)) / "typed-gguf"
    return pathlib.Path.home() / ".local" / "share" / "typed-gguf"
...
def load_registry(path: pathlib.Path | None = None, *,
    ...
    if not isinstance(payload, dict) or payload.get("schema") != SCHEMA \
        ...
        reason = "unexpected schema (not a typed-gguf registry)"
        if not recover:
            raise RegistryCorruptError(
