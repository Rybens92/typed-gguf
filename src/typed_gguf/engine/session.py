...
from typed_gguf.engine.decide import Batch, ContextPlan, PrefillInfo, SessionMeta
...
from typed_gguf.runtime import capability, ctypes_binding, finder, fit
...
STATE_META_SCHEMA = "typed_gguf.state/v1"
...
def capture_llama_logs(runtime: ctypes_binding.Runtime) -> Iterator[list[str]]:
    ...
    * **`llama_log_get` is NOT a no-argument getter in this build.** It is a tail jump to
      ...
      (`uv run typed-gguf ask …` exited 139, faulthandler pointed straight at the call). Production
...
class ModelHandle:
    ...
    def special_tokens(self) -> list[tuple[int, str]]:
        ...
        (`ctypes_binding.special_tokens`); typed-gguf only reports it. Read once and cached: the
...
def open_model(path: str | os.PathLike[str], *, runtime_dir: str | os.PathLike[str] | None = None,
    ...
    (`typed_gguf.runtime.devices`) instead of trusting the flags it passed.
...
def _load_failure(path: pathlib.Path, arch: str | None, attempts: list[str],
    ...
    return ModelArchUnsupportedError(
        ...
        f"the pinned runtime must support the architecture (run `typed-gguf doctor`). "
...
class BackendClaim:
    ...
    A label is not a measurement: `typed_gguf.runtime.devices` reads the device set out of the
...
def backend_claim(*, requested: str | None = None,
    ...
    * `runtime_dir` — the bundle this run **loaded** (`ModelHandle.runtime.directory`): a better
      ...
      while `$TYPED_GGUF_RUNTIME_DIR`/the fit may run another one (the E3 serving run did);
