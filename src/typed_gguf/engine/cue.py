...
* the **catalogue** (`TURN_CLOSERS`): the documented turn-closer strings of the families typed-gguf
...
* the **vocabulary's own attribute table** (`special`, card `t_635124bf`): every token the model
  ...
  `ModelHandle.special_tokens()` hands the table over; typed-gguf only reports it. A vocabulary that
...
#: each entry is a documented end-of-turn token of a family typed-gguf can be pointed at. It is a
