...
* **Offline, registry-free.** A benchmark never resolves a registry alias and never opens a
  ...
  `typed_gguf.registry.*` at import time (the runtime finder is imported lazily, inside the live
  path only).
...
from typed_gguf.errors import RuntimeMissingError, UserError
...
#: Kept as literals so `bench/harness.py` never imports `typed_gguf.schema` (the CLI validates the
...
DEFAULT_OUT_NAME = "typed-gguf-bench-{suite}{suffix}.json"
...
def resolve_model_path(ref: str | None, *, env: Mapping[str, str] | None = None) -> str:
    ...
    candidate = ref or environ.get("TYPED_GGUF_BENCH_MODEL")
    if not candidate:
        raise BenchError(
            "a benchmark needs --model <path.gguf> (or TYPED_GGUF_BENCH_MODEL): benchmarks never "
...
def runtime_roots(*, home: pathlib.Path | None = None,
    ...
    environ = os.environ if env is None else env
    for name in ("TYPED_GGUF_RUNTIME_DIR", "TYPED_GGUF_BENCH_RUNTIME_DIR"):
...
def classify_runtime(directory: pathlib.Path) -> str | None:
    ...
    `typed_gguf.runtime`-free at import time (A-E2-7).
    """
    from typed_gguf.runtime import finder
...
def backend_runtimes(*, home: pathlib.Path | None = None,
    ...
    explicit = (env or os.environ).get("TYPED_GGUF_RUNTIME_DIR")
    candidates: list[pathlib.Path] = []
    if explicit:
...
def backend_unavailable_reason(backend: str) -> str:
    return (f"no local llama.cpp bundle carries {BACKEND_LIBRARIES.get(backend, backend)} "
            f"(benchmarks never download one: run `typed-gguf init --backend {backend}` or point "
            f"TYPED_GGUF_BENCH_RUNTIME_DIR at extracted bundles)")
...
class LiveModel:
    ...
    def __init__(self, spec: ModelSpec, *, states_home: pathlib.Path | None = None,
        ...
        self._states_home = (pathlib.Path(states_home) if states_home
                             else pathlib.Path(tempfile.mkdtemp(prefix="typed-gguf-bench-states-")))
...
@dataclass(frozen=True)
class BenchConfig:
    """One `typed-gguf bench` invocation (the published tables pin every field here)."""
...
def reproduce_command(config: BenchConfig) -> str:
    parts = ["uv run typed-gguf bench --suite", config.suite]
