...
DEV_SET_SCHEMA = "typed_gguf.bench.devset/v1"
PROVENANCE = ("authored for typed-gguf E2 (2026-09-18); no vendor evaluation set is reused")
...
def validate(items: list[DevItem], *, max_words: int = MAX_STATE_WORDS) -> list[str]:
    ...
    for item in items:
        ...
        if not item.provenance or "typed-gguf" not in item.provenance.lower():
