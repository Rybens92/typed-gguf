...
def route(candidates: Sequence[Candidate], *, needs: Needs, host: fit.HostFacts,
    ...
    if not candidates:
        raise ModelNotFoundError(
            ...
            "given (use `typed-gguf models pull` / `typed-gguf models use`, or pass a GGUF path)")
...
def decision_of(answer: Mapping[str, Any]) -> str:
    ...
    `typed-gguf calibrate`'s live measurement (the row's `got` key), which must agree about what the
