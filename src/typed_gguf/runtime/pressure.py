...
sibling cards' campaigns inside it). Every typed-gguf probe is a child process by design
(`typed_gguf.runtime.isolated`: a C library must never be able to kill the command), so the probe
