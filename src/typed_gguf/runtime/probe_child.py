"""Child-process half of `typed_gguf.runtime.isolated` — the disposable probe (E1a FIX t_eae35404)

One JSON request on stdin, one JSON object on stdout. Run as:

    python -m typed_gguf.runtime.probe_child <<< '{"mode": "probe", "runtime_dir": "/…"}'
...
libraries and drivers, and its own teardown is exactly what aborted `typed-gguf init` on the
...
from typed_gguf.runtime import capability, install  # noqa: E402
