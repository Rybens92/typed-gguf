...
Why this module exists: on the operator's RTX host `typed-gguf init --json` printed its JSON and
...
So no typed-gguf command dlopens a bundle. Every probe runs in a disposable child
(`typed_gguf.runtime.probe_child`), **one bundle per process** — which also keeps the rejected
...
CHILD_MODULE = "typed_gguf.runtime.probe_child"
