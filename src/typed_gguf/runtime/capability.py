...
through ctypes — the same check the oracle runs in section B — and is what `typed-gguf doctor`
uses; `deep=False` (or `TYPED_GGUF_DEEP_PROBE=0`) keeps CI able to probe a bundle it cannot
actually dlopen.
"""
...
from typed_gguf.errors import ModelArchUnsupportedError
from typed_gguf.runtime import finder, isolated, pins, pressure
    ...
    def warnings(self) -> list[str]:
        ...
        if self.build is not None and self.expected_tag and self.tag != self.expected_tag:
            out.append(f"runtime build {self.tag} differs from the pinned {self.expected_tag} "
                       f"(the oracle pins the pinned build; re-run `typed-gguf init --force`)")
        if not self.symbols_checked:
            out.append("symbol probe skipped (deep probe disabled via TYPED_GGUF_DEEP_PROBE=0)")
        for name, error in sorted(self.backend_errors.items()):
            out.append(f"W_BACKEND_LOAD: the {name} backend of this bundle does not load on "
                       f"this host ({error}); typed-gguf uses {self.accelerator()} instead "
        ...
        if "vulkan" in self.backends:
            out.append("W_VULKAN_WARMUP: the first Vulkan decode pays the shader compilation "
                       "(23-30 s measured); `typed-gguf init`/`serve` warm up before serving")
        return out
...
def scan_in_process(runtime_dir: str | os.PathLike[str] | pathlib.Path, *,
    ...
    (`typed_gguf.runtime.probe_child`), and what a test drives directly when it wants to inject a
...
#: (`typed_gguf.runtime.isolated`). Tests that inject the dlopen seams below swap it for
...
def deep_probe_enabled(env: dict[str, str] | None = None) -> bool:
    ...
    return env.get("TYPED_GGUF_DEEP_PROBE", "1") not in ("0", "false", "no")
