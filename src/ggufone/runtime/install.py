...
def install(backend: str = "auto", *, home: pathlib.Path | None = None,
    ...
    plan = plan_install(backend, home=home, lock=lock, offline_cache=offline_cache,
                        system=system, machine=machine, **detect_kwargs)
    if dry_run:
        return {"dry_run": True, "plan": plan.to_dict()}

    if plan.dest.exists() and not force:
        record = finder.runtime_record(home)
        return {"already_installed": True, "variant": plan.variant, "dir": str(plan.dest),
                "record": record,
    ...
    probe = capability.probe_runtime(plan.dest, deep=deep_probe, lock=lock,
                                    run_tools=True, system=system)
    warmup_ms: float | None = None
    model_path = warmup_model
    if model_path is None:
        record_prev = finder.runtime_record(home) or {}
        candidate = record_prev.get("warmup_model")
        if candidate and pathlib.Path(candidate).exists():
            model_path = candidate
    warmup_error: str | None = None
    if model_path:
        try:
            warmup_ms = round(warmup(plan.dest, model_path), 3)
        except Exception as exc:  # noqa: BLE001 - warm-up must never fail an install
            warmup_error = f"{exc.__class__.__name__}: {exc}"
    ...
    record = {
        ...
        "libllama_sha256": (sha256_file(plan.dest / finder.library_names(system)["llama"])
                            if (plan.dest / finder.library_names(system)["llama"]).exists()
                            else None),
        "url": url or plan.url,
        "source": source,
        ...
        "warmup_model": str(model_path) if model_path else None,
    ...
    return {
        "variant": plan.variant, "dir": str(plan.dest), "source": source, "record": record,
        "asset_sha256": record["asset_sha256"], "build": probe.build,
        "backends": list(probe.backends), "warmup_ms": warmup_ms,
        "probe_failures": probe.failures(), "probe_warnings": probe.warnings(),
