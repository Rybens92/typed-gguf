"""The process's own teardown policy: who decides how a ggufone process ends (card t_97f1bc93).
...
Measured on the operator's box (`ggufone bench --backend vulkan`, one bundle, a 4B model, a device
...
No ggml, no llama.cpp, no ggufone frame is in it: the fault is the NVIDIA ICD's **own exit
...
def end_process(code: int) -> NoReturn:
    ...
    the bytes the reader came for (`python -m ggufone bench … --json | jq .` must not read a
