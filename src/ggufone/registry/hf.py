...
(`docs/evidence/hf_spark_x2_5.json`), so `GGUFONE_OFFLINE=1` (or any offline unit test) can
...
from ggufone.errors import DownloadError, HfAuthError, InsufficientDiskError, Sha256MismatchError
...
CHUNK = 1 << 20
USER_AGENT = "ggufone/0.1 (+https://github.com/Rybens92/ggufone)"
...
def _translate(exc: Exception, repo: str) -> Exception:
    ...
    if isinstance(exc, (urllib.error.URLError, TimeoutError, OSError)):
        return DownloadError(
            ...
            f"offline-capable only for the pinned default model (GGUFONE_OFFLINE=1)")
    return DownloadError(f"E_DOWNLOAD_FAILED: {repo}: {exc}")


def offline_enabled() -> bool:
    return os.environ.get("GGUFONE_OFFLINE", "0") not in ("0", "", "false", "no")
...
def snapshot_path(repo: str | None = None) -> pathlib.Path | None:
    ...
    override = os.environ.get("GGUFONE_SNAPSHOT")
...
def check_disk_space(dest_dir: str | os.PathLike[str], required_bytes: int, *,
    ...
    if required_bytes > free:
        raise InsufficientDiskError(
            ...
            f"free up space or set GGUFONE_HOME to another filesystem")
    return free
