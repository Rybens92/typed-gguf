# typed-gguf v0.2.2 — typed decisions on any GGUF

> Release notes for the v0.2.2 build. Two things change here, and both are about work that must not
> be lost or misreported: two calls that arrive at the same moment can no longer cost one of them its
> answer while the warm host is starting, and a GPU too tight for the requested context now walks a
> real ladder — a smaller KV cache, a smaller context, fewer GPU layers, CPU-only — instead of
> failing with a message that claimed rescues it never tried. Nothing was re-measured, and no number
> the tool *chooses* moved: the same request on the same box still reaches the same answer.
...
> repository and marked where it was measured. v0.2.1 is live on PyPI as `typed-gguf`.
...
## The headline of this build: two calls at the same moment never cost a caller its answer
...
## The second fix of this build: a tight GPU degrades honestly, and says only what it tried
...
The gates behind both fixes above were tightened: the race gate now waits, bounded, for the losing
host to leave the process table (5 s, 50 ms steps) and its inline double carries a real answer body
instead of an empty one, and the placement pins assert the seven-rung walk through the production
session rather than a hand-made list. Test-only, and no behaviour a user can reach moved.
