...
4. `qwen35moe`'s **template is measured here** (Occamy 1.0, Tiel-Coder), but its **cue is refused**
   ...
   `docs/evidence/e3c_cue_shapes.md`. **Two measurements qualify that sentence, both on the same 60
   committed items**: on the *bench* shape Tiel-Coder answers — 46/60 rows `measured`, coverage
   median 2.58e-01, against Occamy's 3/60 — so the refusal is a property of the shipped *serving*
   shape, not of the arch; and on that serving shape it is **Occamy** whose cue row is
   `<|im_end|>` while **Tiel**'s holds the special token `248069` (mass 0.68–0.99 on 19/20 batch
   rows, `<|im_end|>` on the 20th), so `W_CUE_REFUSED` fires for one model and not the other.
   `docs/evidence/e3c_tiel_t_a58f8b67_tiel.md` §5–§5.1 (pairing proof: the batch's 20 questions are
   byte-identical to dev rows `c01`…`c20`) and `docs/BENCHMARKS.md` §7.4.
