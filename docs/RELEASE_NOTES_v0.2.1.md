# typed-gguf v0.2.1 — typed decisions on any GGUF

> Release notes for the v0.2.1 build. What is new here is one honesty fix in what a context plan
> *says*: a `--n-ctx` pin that had to shrink is now reported `shrunk`, with the note that names the
> window you asked for and the window you got. Nothing was re-measured, and no number the tool
> chooses moved — the context, the KV rung and the budget are computed exactly as before.
...
> repository and marked where it was measured. v0.2.0 is live on PyPI as `typed-gguf`.
...
## The headline of this build: a pin that could not be held now says so

`--n-ctx N` asks for a window of exactly N tokens. When the box cannot hold N, the plan brings the
number down — that has always been the behaviour — but the plan used to keep *calling* the result
`ctx_limit "pinned"`, with no note and no warning: the line said the request had been honoured while
the number beside it was a smaller one. With a runtime bundle installed (the default path) the plan
comes from the binary, and that was the path that relabelled a shrunken pin as "pinned"; the
estimate path already said `shrunk` but stayed silent when the shrinking crossed the standard.

What a reader sees now, on both paths:

- **A pin that had to shrink is reported `ctx_limit "shrunk"`**, never `"pinned"`. A pin that really
  fits still reads `"pinned"`, and an unpinned plan below the standard still reads `"shrunk"` as it
  did.
- **The note names both windows, not just the result** — `n_ctx shrunk <asked> -> <actual> to fit the
  budget (<MiB>); raise --fit-target, lower --fit-ctx or use a smaller quant`. The two numbers are
  the window you asked for and the window you got; the last three names are the knobs that would
  bring them closer together.
- **`W_CTX_BELOW_STANDARD` fires when the shrinking lands under the standard** (32 768 tokens),
  exactly as it does for an unpinned plan. A pin the box capped *above* the standard stays quiet:
  it is below your number, not below the standard.
- **Your chosen numbers never change — only the report.** The context, the KV rung, the layer count
  and the memory budget are computed from the same inputs as before; the label, the note and the
  warning now read the window you *asked* for instead of the already-shrunk one. The load is the
  same load it was, described honestly.

The rule was already written down in the spec — `docs/SPEC-context-v2.md` §5.5: *a pin that had to
shrink reports `ctx_limit "shrunk"`, not `"pinned"`* — the default path simply did not obey it. A
fresh-eyes field test found it live, on a real box, by running the tool the way a user does; the fix
landed with a test at that seam, and both cases were re-run by hand on the operator box before this
release — the pin the box capped, and the pin that shrank below the standard — before → after.
...
- **Offline suite, the shape CI runs** — `env -u PYTHONPATH TYPED_GGUF_TEST_BLOCK_NET=1
  ...
  **1 611 passed, 57 skipped, 0 failed** in 43 s. The skips are the live cases that want a GPU, a
  model file or `--run-network`; nothing in the suite touches the network. The two tests the fix in
  this build added at that seam are inside those 1 611 (1 609 was the v0.2.0 count; the pin fix added
  the two, and nothing else moved).
- **The two release gates.** `tests/test_public_docs.py` (17 tests) pins this file, the README and
  ...
  checkout, `uv run typed-gguf version` prints `typed-gguf 0.2.1`; the workflow's own gate script,
  run against this build's artifacts (`typed_gguf-0.2.1-py3-none-any.whl` +
  `typed_gguf-0.2.1.tar.gz`), accepts the tag `v0.2.1` and refuses `v0.2.0` and `v0.2.2`.
- **The pin-shrink fix, verified live before the release.** The two cases the field test found were
  re-run by hand on the operator box, before → after: a `--n-ctx` the box capped *above* the standard
  (the label was the only thing that moved — no warning, by design) and a `--n-ctx` that shrank below
  it (`ctx_limit shrunk` + `W_CTX_BELOW_STANDARD` + the note naming both numbers). Its changed lines
  were also checked by hand-mutation rather than a sweep: eight mutations of the label, the note and
  the warning, none surviving once the two boundary tests were pinned. The plan's arithmetic is
  untouched, so the v2 receipts below still describe this build.
...
- **Mutation testing.** Not run for this build: it moves a version string, these notes and the two
  version pins, and no product line at all, so a sweep would score the v0.2.0 tree over again. That
  sweep — over the module this release reports on (`src/typed_gguf/runtime/fit.py`, 2 267 mutants,
  soft threshold) — stands as published: 1 335 killed / 805 survived = **62.4 %**, receipt
